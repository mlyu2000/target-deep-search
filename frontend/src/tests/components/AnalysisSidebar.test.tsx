...
describe('AnalysisSidebar', () => {
  it('renders the three analysis buttons and reports entity count', () => {
    render(<AnalysisSidebar graphData={graph} activeView="competitive" onSelect={() => {}} />)
  ...
  it('calls onSelect with the chosen view when clicked', () => {
    ...
    render(<AnalysisSidebar graphData={graph} activeView="competitive" onSelect={onSelect} />)
    ...
    fireEvent.click(screen.getByText('What-if Simulation'))
    expect(onSelect).toHaveBeenCalledWith('whatif')
  })
})
